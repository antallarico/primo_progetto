...
class FormazioneHelper
{
    public static function calcolaStatoFormazione($lavoratore_id, $competenza)
    {
        ...
        if ($corso_rappresentativo->soglia_ore_rolling) {
			...
			$prossima_scadenza_ore = null;
			// Calcola la prima partecipazione che fa cadere sotto soglia
			$partecipazioni_ordinate = $partecipazioni->sortBy('data_formazione');

			$somma_parziale = 0;
			foreach ($partecipazioni_ordinate as $p) {
			$data = \Carbon\Carbon::parse($p->data_formazione);
			if ($data->between($data_limite, $oggi)) {
				$somma_parziale += $p->sessione?->durata_effettiva ?? 0;
			}

			if ($somma_parziale >= $corso_rappresentativo->soglia_ore_rolling) {
        // Quando raggiungo soglia, memorizzo la data limite di questa partecipazione
        $prossima_scadenza_ore = $data->addYears($corso_rappresentativo->rolling_finestra_anni);
        break;
    }
}
            ...
            return [
				'stato' => $ore_valide >= $corso_rappresentativo->soglia_ore_rolling ? 'valida' : 'scaduta',
				'scadenza' => null,
				'ore_valide' => $ore_valide,
				'prossima_uscita' => $prossima_scadenza_ore?->format('Y-m-d')
			];
